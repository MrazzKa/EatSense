...
1. Start the mock OpenAI server:
   ...
   ```bash
   node load-tests/mock-openai-server.js
   ```
...
3. Run the k6 scenario you want:
   ...
   ```bash
   # 100 RPS for 30s
   k6 run -e BASE_URL=http://localhost:3000 -e TARGET_RPS=100 load-tests/food-analyze.js

   # ramp 100 → 500 → 1000 RPS, 1 min each
   k6 run -e BASE_URL=http://localhost:3000 load-tests/food-analyze-ramp.js
   ```

## Auth

The endpoint is behind JWT. For load tests we generate a single test user and
bake its token into `K6_AUTH_TOKEN`. See `load-tests/seed-test-user.ts` for
...
Each run prints standard k6 metrics + a CSV in `load-tests/results/`. Compare
