...
export default function ConsultationsListScreen({ navigation }) {
  ...
  const getStatusColor = (consultation) => {
    ...
    const daysRemaining = Math.ceil((new Date(consultation.endsAt) - new Date()) / (1000 * 60 * 60 * 24));
  ...
  const getStatusText = (consultation) => {
    if (consultation.status !== 'active') return 'Completed';
    const daysRemaining = Math.ceil((new Date(consultation.endsAt) - new Date()) / (1000 * 60 * 60 * 24));
  ...
  const renderConsultation = ({ item }) => (
    <TouchableOpacity style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]} onPress={() => navigation.navigate('Chat', { consultationId: item.id })}>
      <View style={styles.cardHeader}>
        <View style={[styles.avatarPlaceholder, { backgroundColor: colors.primary + '20' }]}>
          <Ionicons name="person" size={24} color={colors.primary} />
        </View>
        <View style={styles.cardInfo}>
          <Text style={[styles.specialistName, { color: colors.textPrimary }]}>{item.specialist?.displayName || 'Specialist'}</Text>
          <Text style={[styles.specialistType, { color: colors.textSecondary }]}>{item.specialist?.type === 'dietitian' ? 'Dietitian' : 'Nutritionist'}</Text>
        </View>
      ...
      {item.messages?.[0] && (
        <Text style={[styles.lastMessage, { color: colors.textSecondary }]} numberOfLines={2}>{item.messages[0].content}</Text>
      )}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>No consultations yet</Text>
            <TouchableOpacity style={[styles.findButton, { backgroundColor: colors.primary }]} onPress={() => navigation.navigate('SpecialistList')}>
...
const styles = StyleSheet.create({
  ...
  specialistName: { fontSize: 16, fontWeight: '600' },
  specialistType: { fontSize: 13, marginTop: 2 },
  statusContainer: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  statusDot: { width: 8, height: 8, borderRadius: 4 },
  statusText: { fontSize: 12, fontWeight: '500' },
  lastMessage: { fontSize: 14, marginTop: 12, lineHeight: 20 },
  ...
  findButton: { paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12 },
