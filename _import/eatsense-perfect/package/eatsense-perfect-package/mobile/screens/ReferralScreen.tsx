...
export default function ReferralScreen({ navigation }) {
  ...
  const loadStats = async () => {
    try {
      const data = await ReferralsService.getMyReferrals();
      setStats(data);
    } catch (error) {
  ...
  const handleShare = useCallback(async () => {
    if (!stats?.code) return;
    const message = `Join me on EatSense! Use my code ${stats.code} and we both get 7 days PRO free! Download: https://eatsense.app`;
    try {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.textPrimary }]}>{t('referral.title') || 'Invite Friends'}</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={[styles.heroCard, { backgroundColor: colors.primary }]}>
          ...
          <Text style={styles.heroSubtitle}>Invite friends and both of you get 7 days of PRO free!</Text>
        </View>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={[styles.statBox, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Text style={[styles.statLabel, { color: colors.textSecondary }]}>Days earned</Text>
          </View>
        </View>

        {stats?.referrals?.length > 0 && (
          <View style={[styles.historySection, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Text style={[styles.historyTitle, { color: colors.textPrimary }]}>Recent invites</Text>
            {stats.referrals.map((referral) => (
              <View key={referral.id} style={styles.historyItem}>
                <View style={[styles.historyAvatar, { backgroundColor: colors.primary + '20' }]}>
                  <Text style={[styles.historyAvatarText, { color: colors.primary }]}>{referral.friend.firstName?.charAt(0).toUpperCase() || '?'}</Text>
                </View>
                <View style={styles.historyInfo}>
                  <Text style={[styles.historyName, { color: colors.textPrimary }]}>{referral.friend.firstName || 'Friend'}</Text>
                  <Text style={[styles.historyDate, { color: colors.textSecondary }]}>{new Date(referral.date).toLocaleDateString()}</Text>
                </View>
                <View style={[styles.historyBadge, { backgroundColor: '#4CAF50' + '20' }]}>
                  <Text style={[styles.historyBadgeText, { color: '#4CAF50' }]}>+{referral.bonusDays} days</Text>
                </View>
              </View>
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  heroIcon: { width: 70, height: 70, borderRadius: 35, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  heroTitle: { color: '#fff', fontSize: 22, fontWeight: '700', marginBottom: 8, textAlign: 'center' },
  heroSubtitle: { color: 'rgba(255,255,255,0.9)', fontSize: 14, textAlign: 'center', lineHeight: 20 },
  codeCard: { borderRadius: 12, padding: 16, marginBottom: 16, borderWidth: 1 },
  ...
  shareButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 14, borderRadius: 12, gap: 8 },
  shareButtonText: { color: '#fff', fontSize: 16, fontWeight: '600' },
  statsRow: { flexDirection: 'row', gap: 12, marginBottom: 16 },
  statBox: { flex: 1, alignItems: 'center', padding: 16, borderRadius: 12, borderWidth: 1 },
  statValue: { fontSize: 28, fontWeight: '700', marginTop: 8 },
  statLabel: { fontSize: 12, marginTop: 4 },
  historySection: { borderRadius: 12, padding: 16, marginBottom: 16, borderWidth: 1 },
  ...
  historyBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4 },
  historyBadgeText: { fontSize: 12, fontWeight: '600' },
  howItWorks: { borderRadius: 12, padding: 16, borderWidth: 1 },
  howItWorksTitle: { fontSize: 16, fontWeight: '600', marginBottom: 16 },
  ...
  stepText: { flex: 1, marginLeft: 12, fontSize: 14, lineHeight: 20 },
});
