...
export default function ChatScreen({ navigation, route }: ChatScreenProps) {
    ...
    const handleSend = useCallback(async () => {
        ...
        try {
            ...
            Alert.alert(t('common.error'), t('errors.sendMessage'));
        } finally {
            setSending(false);
        }
            ...
            { text: t('common.cancel'), style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert(t('common.error'), t('errors.shareMeals'));
                    }
                },
            },
        ]);
    ...
    const renderMessage = ({ item }: { item: any }) => {
        ...
        return (
            <View style={[styles.messageRow, isMe ? styles.messageRowRight : styles.messageRowLeft]}>
                ...
                >
                    {item.type === 'meal_share' && (
                        <View style={styles.mealShareBadge}>
                            <Ionicons name="restaurant" size={14} color={isMe ? '#fff' : colors.primary} />
                            <Text style={[styles.mealShareText, { color: isMe ? '#fff' : colors.primary }]}>
                                {t('chat.shareMeals')}
                            </Text>
                        </View>
                    )}
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
            <View style={[styles.header, { borderBottomColor: colors.border }]}>
                ...
                <View style={styles.headerCenter}>
                    <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>
                        {consultation?.specialist?.displayName || 'Specialist'}
                    </Text>
                    <Text style={[styles.headerSubtitle, { color: isActive ? '#4CAF50' : colors.textSecondary }]}>
                        {isActive ? `${daysRemaining} ${t('chat.daysRemaining')}` : t('chat.consultationEnded')}
                    </Text>
                </View>
                <TouchableOpacity onPress={handleShareMeals}>
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyContainer}>
                        <Text style={[styles.emptyText, { color: colors.textSecondary }]}>{t('chat.startConversation')}</Text>
                    </View>
                }
            />
            ...
            {isActive ? (
                ...
                <View style={[styles.endedBanner, { backgroundColor: colors.surface }]}>
                    <Text style={[styles.endedText, { color: colors.textSecondary }]}>{t('chat.consultationEnded')}</Text>
                </View>
            )}
        </SafeAreaView>
    );
