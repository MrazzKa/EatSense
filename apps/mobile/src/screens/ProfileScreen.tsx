...
const ProfileScreen = () => {
  ...
  const handleNotificationToggle = async (value) => {
    try {
      ...
      setNotificationPreferences(prev => ({
        ...
        dailyPushMinute: typeof updated.dailyPushMinute === 'number' ? updated.dailyPushMinute : notificationPreferences.dailyPushMinute,
  ...
  const saveSmartTips = async (partial: { smartTipsEnabled?: boolean; smartTipsHour?: number; smartTipsMinute?: number; smartTipsMedicalAllowed?: boolean; healthIssues?: string[] }) => {
    try {
      ...
      // Smart tips are delivered as remote push from the backend scheduler, so we
      // need the Expo push token registered in the API. The app registers it on
      // launch via usePushNotifications, but if the user denied permission on
      // first launch and now flipped Smart Tips on, that registration never
      // happened — register it here so the very first tip can actually arrive.
      if (wantsEnable) {
        try {
          if (Device.isDevice) {
            const projectId =
            ...
            if (projectId) {
              ...
              if (tokenResp?.data) {
                const deviceId = Constants.sessionId || Constants.deviceId || 'unknown';
          ...
          console.warn('[smartTips] getExpoPushTokenAsync failed', tokenErr);
        }
      }
  ...
  const toggleHealthIssue = (issue: string) => {
    ...
    saveSmartTips({
      healthIssues: next,
      smartTipsEnabled: notificationPreferences.smartTipsEnabled && next.length > 0,
    });
  };
    ...
    <>
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: tokens.colors.background }}>
          <View style={[styles.editModalHeader, { borderBottomWidth: 0, paddingTop: Math.max(insets.top, 20) }]}>
            <TouchableOpacity
      ...
      {/* Time Picker */}
      {
        showTimePicker && (
          <>
            {Platform.OS === 'ios' ? (
              <Modal transparent animationType="fade" visible={showTimePicker} statusBarTranslucent>
                <TouchableOpacity
                ...
                >
                  <View style={[styles.modalContent, { padding: 0, paddingBottom: Math.max(insets.bottom, 30) }]}>
                    <View style={[styles.modalHeader, { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 10 }]}>
                      ...
                      <TouchableOpacity onPress={confirmIosTime} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
