        ...
        try {
          ...
          if (response?.answer) {
            if (Array.isArray(response?.suggestions)) {
              setQuickSuggestions(response.suggestions.filter(Boolean).slice(0, 3));
            }
    ...
    >
      ...
      >
        <View style={[styles.container, { backgroundColor: colors.background || colors.surface }]}>
          ...
          {!isLoading && quickSuggestions.length > 0 && (
            <View style={styles.suggestionsRow}>
              {quickSuggestions.map((suggestion) => (
                <TouchableOpacity
                  key={suggestion}
                  style={[styles.suggestionChip, { backgroundColor: colors.surfaceMuted || '#F2F2F7', borderColor: colors.border || '#E5E5EA' }]}
                  onPress={() => handleSuggestionPress(suggestion)}
                  activeOpacity={0.85}
                >
                  <Text style={[styles.suggestionText, { color: colors.textPrimary || colors.text || '#111' }]} numberOfLines={1}>
                    {suggestion}
                  </Text>
                </TouchableOpacity>
              ))}
